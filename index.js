	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Menos um corno @${num.split('@')[0]}🖕`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				só: {
					...
					dono: '❌ Este comando só pode ser usado pelo dono do grupo! ❌',
					donoB: '❌ Este comando só pode ser usado pelo proprietário do bot ❌',
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) Retorna reply(mess.error.stick)
				...
				case 'gtts':
					...
					break
				case 'shit':
					meme = await fetchJson()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT2b298666q98h89saH7maYzmnH2KxD49cudw&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
					break
				case '(PORNHUB)':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`(https://pt.pornhub.com/view_video.php?viewkey=ph5d4b22ba4a2cc)`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '(Legenda)'})
					break
				case '(OI)':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT8UbQkdM6VFTFs9FSC2BTcJCb6aQT-1gYsmQ&usqp=CAU)`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '(Legenda)'})
					break
				case 'setprefix':
